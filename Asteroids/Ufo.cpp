...
void Ufo::Update(System *system)
{
	...
	angle_ = Maths::WrapModulo(angle_, Maths::TWO_PI);	
}
...
void Ufo::Render(Graphics *graphics) const
{
	...
	ImmediateModeVertex square[7] =
	{
		{-2.0f, -1.0f, 0.0f, 0xffffffff},
		{-1.0f,  0.5f, 0.0f, 0xffffffff},
	...
	XMMATRIX UfoTransform = scaleMatrix *
		rotationMatrix *
		translationMatrix;

	ImmediateMode *immediateGraphics = graphics->GetImmediateMode();

	immediateGraphics->SetModelMatrix(UfoTransform);
